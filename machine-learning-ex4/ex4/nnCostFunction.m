...
% map vector y to logistic matrix Ynn
Ynn = to_logistic_matrix(num_labels, y);
...
J = 1/m *( sum(sum(-Ynn .* log(h))) + sum(sum( -(1-Ynn) .* log(1-h) )) );

% Regularized cost function, do NOT regularize bais
temp1 = Theta1;
temp1(:, 1) = 0;
temp2 = Theta2;
temp2(:, 1) = 0;
...
function [Ynn] = to_logistic_matrix(num_labels, y)
% map vector y to matrix y
labels = [1:num_labels];
m = size(y,1);
Ynn = zeros(m, num_labels);
for i = 1:m
    Ynn(i, :) = y(i) == labels;
end
